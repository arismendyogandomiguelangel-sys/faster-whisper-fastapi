...
class PillOverlay:
    ...
    def _draw_frame(self):
        ...
        self._draw_rounded_rect(2, 2, w-2, h-2, r, fill=TRANSPARENT_COLOR, outline=border_color, width=2)
        self._draw_rounded_rect(4, 4, w-4, h-4, r-2, fill="#0a0a15", outline="", width=0)
    ...
    def show(self, state=None):
        ...
        elif self.state == self.STATE_HIDDEN: self.state = self.STATE_IDLE
        self.status_text = ""
        self.root.deiconify()
        self._cancel_fade()
    ...
    def set_state(self, state, text=""):
        ...
        if state == self.STATE_HIDDEN: self.hide()
        else:
            self.root.deiconify()
            self._cancel_fade()
    ...
    def _fade_tick(self):
        if self._fade_seconds <= 0:
            self.hide()
            return
        if self.state == self.STATE_LISTENING: return
...
class TeoigoClient:
    ...
    def toggle_recording(self):
        with self._lock:
            if self.is_recording: self._stop_and_transcribe()
    ...
    def _start_recording(self):
        ...
        try:
            self.stream = sd.InputStream(
                ...
                dtype='int16', callback=audio_callback, blocksize=512,
            )
            self.stream.start()
            ...
            self.overlay.set_state(PillOverlay.STATE_ERROR, "mic error")
            self.overlay.start_fade_timer(5)
    ...
    def _stop_and_transcribe(self):
        ...
        self.overlay.set_state(PillOverlay.STATE_PROCESSING, "transcribiendo...")
        ...
        wav_buffer.seek(0)
        log(f"WAV generado: {wav_buffer.getbuffer().nbytes / 1024:.0f} KB")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if text and str(text).strip():
                    ...
                    log("Sin habla detectada.")
                    self.overlay.set_state(PillOverlay.STATE_ERROR, "sin habla")
                    self.overlay.start_fade_timer(4)
            elif response.status_code == 401:
                log("ERROR: API KEY invalida!")
                self.overlay.set_state(PillOverlay.STATE_ERROR, "api key!")
                self.overlay.start_fade_timer(5)
            else:
                log(f"ERROR servidor: {response.status_code}")
                self.overlay.set_state(PillOverlay.STATE_ERROR, f"error {response.status_code}")
                self.overlay.start_fade_timer(5)
        except requests.exceptions.SSLError as e:
            log(f"ERROR SSL: {e}")
            log("Reintentando sin SSL...")
            try:
                wav_buffer.seek(0)
                resp2 = requests.post(
                    WHISPER_URL,
                ...
                if resp2.status_code == 200:
                    text = str(resp2.json()).strip()
                    if text:
                        pyperclip.copy(text)
                        time.sleep(0.1)
                        pyautogui.hotkey('ctrl', 'v')
                        log(f'TRANSCRITO (no-ssl): "{text[:80]}"')
                        self.overlay.set_state(PillOverlay.STATE_IDLE)
                        self.overlay.start_fade_timer(5)
                        return
            except: pass
            self.overlay.set_state(PillOverlay.STATE_ERROR, "SSL error")
            self.overlay.start_fade_timer(5)
        except requests.exceptions.ConnectionError:
            log("ERROR: Sin conexion")
            self.overlay.set_state(PillOverlay.STATE_ERROR, "sin conexion")
            self.overlay.start_fade_timer(5)
        except requests.exceptions.Timeout:
            log("ERROR: Timeout")
            self.overlay.set_state(PillOverlay.STATE_ERROR, "timeout")
            self.overlay.start_fade_timer(5)
        except Exception as e:
            log(f"ERROR: {e}")
            traceback.print_exc()
            self.overlay.set_state(PillOverlay.STATE_ERROR, "error")
            self.overlay.start_fade_timer(5)
